    ...
    return (
        <div className='container' style={{ paddingTop: "30px" }}>
            ...
            <div className='horizontal-paddings-15'>
                <div className='direction row justify-content-start'>
                    {
                        list.map(i =>
                            <ReptileItem
                                key={i.id}
            ...
            <footer>
                <nav className="pagination-color" aria-label="Reptile Pagination" style={{ marginTop: "15px", marginBottom: "25px" }}>
                    <ul className="pagination justify-content-center">
                        <li className={"front page-item" + (page == 0 ? " disabled" : "")}><a className="front page-link" onClick={() => { if (page > 0) setPage(page - 1); }}>წინა</a></li>
                        {
                            Array.from(Array(pages).keys()).map(i =>
                                <li className={i == page ? "page-item active" : " frontpage-item"} onClick={() => setPage(i)}><a className="front page-link">{i + 1}</a></li>
                            )
                        }
                        <li className={"page-item" + (page == pages - 1 ? " disabled" : "")}><a className=" front page-link" onClick={() => { if (page < pages - 1) setPage(page + 1); }}>შემდეგი</a></li>
                    </ul>
                </nav>
            </footer>
        </div >
